...
import { live, table, ui, type Prompt } from './state.svelte';
...
export function promptAct(act: string) {
  ...
  if (p.kind === 'respond') table.respond(act as RespondAction);
  else if (p.kind === 'dez') table.decideDez(act as DezAction);
  else if (act === 'new') newGame();
}

export function newGame() { ui.log = []; table.newGame(); }
...
export function onTableChange(snap: TableSnapshot, events: GameEvent[]) {
  ...
  for (const e of events) {
    const line = formatEvent(e, snap.game);
  ...
  if (snap.seat !== prev.seat) setView(snap.seat);
